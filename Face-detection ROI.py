...
while(True): #Jeden Frame laden
    ...
    try:
        ...
        if nummer == 1:
            try:
                faces, y, x, w, h = detect_faces(cascade, frame)
                height = y
                x_coor = x
            except:
                nummer -= 1
        ...
        break
    if height_list != [] and x_list != []:
        if height_list[-1] == 0:
            case = 1
        elif height_list[-1] < half_window_rectangle and x_list[-1] < half_window_rectangle:
            case = 1
            frame = frame[0: height + half_window_rectangle+ h, 0: x_coor + half_window_rectangle + w]
        elif height_list[-1] < half_window_rectangle:
            case = 3
            frame = frame[0: height + half_window_rectangle + h, x_coor - half_window_rectangle: x_coor + half_window_rectangle + w]
        elif x_list[-1] < half_window_rectangle:
            case = 2
            frame = frame[height - half_window_rectangle: height + half_window_rectangle+h, 0: x_coor + half_window_rectangle + w]
        else:
            case = 0
            frame = frame[height - half_window_rectangle: height + half_window_rectangle+h, x_coor - half_window_rectangle: x_coor + half_window_rectangle+w]
    else:
        case = 1
    #Koordinaten in Absolutkoordinaten übersetzen
    if case == 1:
        try:
            faces, y, x, w, h = detect_faces(cascade, frame)
            height = y
        ...
        try:
            faces, y, x, w, h = detect_faces(cascade, frame)
            x_coor = x_coor - half_window_rectangle + x
            height = y
        except:
            faces = frame
            try:
        ...
        try:
            faces, y, x, w, h = detect_faces(cascade, frame)
            x_coor = x
            height = height - half_window_rectangle + y
        except:
            faces = frame
            try:
                height = height_list[-1]
...
for i in writing_list: #Bilder für chosen_pics aussuchen
    cap = cv2.VideoCapture(path)
    cap.set(1, i + ausgleich)  #i steht für Framenummer welche Geladen werden soll
    ret, img = cap.read()
    ...
    '''
    cv2.imwrite(r'C:\...\frame{}.jpg'.format(i + ausgleich), img) #abspeichern
